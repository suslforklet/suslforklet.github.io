...
function setupAddToCartButtons() {
    ...
    document.querySelectorAll('.add-to-cart-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const quantity = window.quantitySelections?.[itemId] || 1;
            const result = addToCart(itemId, quantity);
            ...
            if (result.success) {
                ...
                // Reset quantity to 1
                window.resetQuantitySelector?.(itemId);
            } else {
...
/**
 * Store quantity selections (itemId -> quantity)
 */
if (!window.quantitySelections) {
    window.quantitySelections = {};
}
...
window.increaseQuantity = function(itemId) {
    window.quantitySelections[itemId] = (window.quantitySelections[itemId] || 1) + 1;
    window.updateQuantityDisplay(itemId);
};

/**
 * Decrease quantity for an item (minimum 1)
 * @param {string} itemId - Item ID
 */
window.decreaseQuantity = function(itemId) {
    const current = window.quantitySelections[itemId] || 1;
    if (current > 1) {
        window.quantitySelections[itemId] = current - 1;
        window.updateQuantityDisplay(itemId);
    }
};
...
window.updateQuantityDisplay = function(itemId) {
    ...
    const minusBtn = document.querySelector(`.qty-minus[data-item-id="${itemId}"]`);
    
    if (display) {
        display.textContent = window.quantitySelections[itemId] || 1;
    }
    
    // Disable minus button if quantity is 1
    if (minusBtn) {
        minusBtn.disabled = (window.quantitySelections[itemId] || 1) <= 1;
    }
};
 ...
 * @param {string} itemId - Item ID
 */
window.resetQuantitySelector = function(itemId) {
    window.quantitySelections[itemId] = 1;
    window.updateQuantityDisplay(itemId);
};
...
function handlePlaceOrder() {
    ...
    if (!currentUser || currentUser.role !== USER_ROLES.CUSTOMER) {
        showToast('Please login as a customer to place an order', 'warning');
        setTimeout(() => {
